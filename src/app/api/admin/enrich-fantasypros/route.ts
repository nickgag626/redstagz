...
export async function POST(req: Request) {
  ...
  // Get players currently in roster for this league/team.
  ...
  const htmlRes = await fetch('https://www.fantasypros.com/mlb/rankings/overall.php', {
    headers: {
      'user-agent': 'Mozilla/5.0 (compatible; redstagz/1.0; +https://redstagz.vercel.app)',
  ...
  for (const p of fpPlayers) {
    ...
    if (!key) continue
    // take the best (lowest ECR) if duplicates
