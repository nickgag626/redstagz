...
export async function POST(req: Request) {
  try {
    ...
    // Parse "Round X" blocks and picks like "5. Aaron Judge" or "8. Shohei Ohtani (Batter)"
    ...
    for (const line of lines) {
      ...
      // pick line: "5. Aaron Judge" optionally followed by unicode etc.
      const mPick = line.match(/^\d+\.\s+(.+?)\s+(?:\uE03E|\(|[A-Z]{2,3}\s*-|$)/) // stop before icons/team bracket if present
      if (mPick && currentRound) {
        let rawName = mPick[1]
        // strip parentheticals like "(Batter)" "(Pitcher)" "DTD"
    ...
    // pull all players so we can update by name
